...
export default function AIOfficeDocumentsFolderPage() {
  ...
  const loadFolder = useCallback(async () => {
    ...
    try {
      ...
      const data = await getOfficeDocumentFolder(folderId);
      setFolder(data);
    } catch (err: any) {
      openSnackbar({
        open: true,
  ...
  }, [folderId, page, limit, search, sortBy, sortOrder]);
  ...
  const openEdit = (row: AiOfficeDocument) => {
    setEditId(row.id);
    setEditInitial(row);
    setFormOpen(true);
  };

  const requestDelete = (row: AiOfficeDocument) => {
  ...
  const handleDownload = async (doc: AiOfficeDocument) => {
    try {
      ...
      a.download =
        ...
        `${(doc.originalName || doc.name || 'documento').replace(/[\\/:*?"<>|]/g, '_')}`;
  ...
  const folderDisplayName = useMemo(() => {
    if (!folder) return 'Carregando...';
    if (isGeneralOfficeFolder(folder)) return 'Geral';
    return folder.name || 'Documentos';
  }, [folder]);
    ...
    () => (
      <Stack direction="row" spacing={1} alignItems="center">
        <Button
          ...
          onClick={() => navigate('/ai/office-documents')}
          sx={{ mr: 1 }}
        >
          Voltar
        </Button>
                      ...
                      <Table size="small" sx={{ '& td, & th': { whiteSpace: 'nowrap' } }}>
                        ...
                        <TableBody>
                          ...
                          {!items.length && (
                            <TableRow>
                              <TableCell
                                colSpan={hasAnyPermission(['office-documents.update', 'office-documents.delete']) ? 5 : 4}
                              >
                                <Stack alignItems="center" sx={{ py: 6 }}>
                                  <Typography variant="body2" color="text.secondary">
                                    {loading ? 'Carregando...' : 'Nenhum documento encontrado.'}
        ...
        <ConfirmDeleteDialog
          ...
          onConfirm={async () => {
            ...
            try {
              ...
              openSnackbar({
                open: true,
                message: res.message || 'Documento removido!',
                variant: 'alert',
                alert: { color: 'success' }
              } as any);
          ...
          description={
            <span>
              Esta ação <b>não pode ser desfeita</b>. Deseja remover o documento <b>{deleteTarget?.name}</b>?
            </span>
          }
        />
              ...
              (() => {
                ...
                if (fileType === 'pdf') {
                  ...
                  return (
                    <Stack
                      spacing={2}
                      sx={{ p: 3, alignItems: 'center', justifyContent: 'center', minHeight: 400 }}
                    >
