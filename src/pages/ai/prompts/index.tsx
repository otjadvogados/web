...
import { listPromptFolders, type PromptFolder, type Prompt } from 'api/prompts';
...
export default function PromptsPage() {
  ...
  const [allFolders, setAllFolders] = useState<PromptFolder[]>([]);
  ...
  const handleFolderClick = (folderId: string) => {
    navigate(`/ai/prompts/${folderId}`);
  };

  const applySearchFilter = (foldersArray: PromptFolder[], q: string) => {
  ...
  const loadFolders = async () => {
    try {
      setLoading(true);
      const foldersArray = await listPromptFolders();
  ...
  const totalPrompts = useMemo(() => {
    return folders.reduce((acc, folder) => acc + (folder.items?.length || 0), 0);
  }, [folders]);
              ...
              {loading ? (
                ...
                <Stack alignItems="center" sx={{ py: 6 }}>
                  <Typography variant="body2" color="text.secondary">
                    {search ? 'Nenhuma pasta encontrada.' : 'Nenhuma pasta ainda. Crie prompts para começar.'}
                  </Typography>
                </Stack>
              ) : (
                <>
                  ...
                  <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 0.5, mt: 0.5 }}>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      {totalFolders} {totalFolders === 1 ? 'pasta' : 'pastas'} • {totalPrompts} {totalPrompts === 1 ? 'prompt' : 'prompts'}
                    </Typography>
                  </Stack>
                  ...
                  >
                    {folders.map((folder) => {
                      // Converte PromptFolder para formato esperado pelo FolderTile
                      const folderForTile = {
                        ...folder,
                        items: folder.items || []
                      };
